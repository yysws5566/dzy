...
try:
    ...
    print('='*100)
    print('策略一初选股实时行情（修正版）')
    print('='*100)
    print(f"{'代码':<8} {'名称':<10} {'现价':>8} {'涨跌幅':>8} {'换手率':>8} {'成交额(亿)':>10} {'流通市值(亿)':>12}")
    print('-'*100)
    ...
    for line in lines:
        if '~' in line:
            ...
            if len(fields) > 50:
                ...
                # 成交额字段37是万元，转为亿
                amount_yi = float(fields[37]) / 10000 if fields[37] else 0
                # 流通市值字段44是亿元
                circ_mv_yi = float(fields[44]) if fields[44] else 0
                ...
                print(f"{code:<8} {name:<10} {price:>8.2f} {pct:>+7.2f}% {turnover:>7.2f}% {amount_yi:>10.2f} {circ_mv_yi:>12.2f}")
    ...
    print(f"\n[PASS] 条件1（股价<20元、流通市值10-200亿、非急拉超5%）：通过 {len(filtered1)} 只")
    for r in filtered1:
    ...
    print(f"\n[PASS] 优选条件（股价5-15元、流通市值<80亿）：{len(filtered2)} 只")
    for r in filtered2:
        print(f"   >> {r['code']} {r['name']} 现价:{r['price']:.2f} 涨幅:{r['pct']:+.2f}% 流通市值:{r['circ_mv_yi']:.1f}亿")
    ...
    print(f"\n[PASS] 稳健条件（股价8-18元、流通市值<100亿、换手率1-6%）：{len(filtered3)} 只")
    for r in filtered3:
    ...
    for r in results:
        if r['code'] not in filtered_codes:
            ...
            print(f"   FAIL {r['code']} {r['name']}: {', '.join(reasons)}")
